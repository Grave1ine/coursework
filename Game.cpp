...
void Game::BuildingGameSubModels() {

    switch (state)             //выбор между меню и игрой
    {
        case MENU:
            PrintMenu(game->menu);
            break;
...
Game::Game() {
    ...
    game->board = BuildBoard(
            game->gameSize);      //присваивает значение BuildBoard в указатель board который лежит в динамической памяти структуры Game
    ...
    switch (game->state)             //выбор между меню и игрой
    {
        ...
        case BOARD:
            ...
            switch (ch) {
                ...
                case 32: //spase
                    return RunPearson(game->pearson, Pearson::SPACE);       //прописывается пробел
                case -1:
                    return RunPearson(game->pearson, game->pearson->direction);
                default:
                    return game->state;
            }
        case EXIT:
